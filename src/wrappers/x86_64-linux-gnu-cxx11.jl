# Autogenerated wrapper script for libcxxwrap_julia_jll for x86_64-linux-gnu-cxx11
export libexcept, libcxxwrap_julia, libpointer_modification, libfunctions, libtypes, libhello, libjlcxx_containers, libbasic_types, libparametric, libinheritance, libcxxwrap_julia_stl, libextended
...
const libexcept_splitpath = ["lib", "libexcept.so"]
...
const libpointer_modification_splitpath = ["lib", "libpointer_modification.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libpointer_modification = "libpointer_modification.so"


# Relative path to `libfunctions`
const libfunctions_splitpath = ["lib", "libfunctions.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libfunctions = "libfunctions.so"


# Relative path to `libtypes`
const libtypes_splitpath = ["lib", "libtypes.so"]
...
const libjlcxx_containers_splitpath = ["lib", "libjlcxx_containers.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libjlcxx_containers = "libjlcxx_containers.so"


# Relative path to `libbasic_types`
const libbasic_types_splitpath = ["lib", "libbasic_types.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libbasic_types = "libbasic_types.so"


# Relative path to `libparametric`
const libparametric_splitpath = ["lib", "libparametric.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libparametric = "libparametric.so"


# Relative path to `libinheritance`
const libinheritance_splitpath = ["lib", "libinheritance.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libinheritance = "libinheritance.so"
...
const libextended_splitpath = ["lib", "libextended.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libextended = "libextended.so"
